...
##### DHCP
После включения, виртуальная машина должна получить сетевой адрес и параметры, необходимые для получения загрузчика, для этого требуется настройка DHCP-сервера. В нашем случае, мы будем использовать _isc-dhcp-server_ со следующими настройками:
```
